...
def project_bulk_delete(request):
    ...
    for project_id in ids:
        ...
        try:
            project.delete()
            messages.success(request, _("%(project)s deleted.") % {"project": project})
...
def create_task(request, stage_id):
    ...
    if request.user.employee_get == project.manager or request.user.has_perm(
...
def drag_and_drop_task(request):
    ...
    if (
        ...
        or request.user.employee_get == task.task_manager
        or request.user.employee_get in task.task_members.all()
        or request.user.employee_get == project.manager
        ...
        for key, val in sequence.items():
            if Task.objects.get(id=key).sequence != val:
                ...
                change = True
        return JsonResponse(
            {"type": "success", "message": _("Task stage updated"), "change": change}
        )
    change = True
    return JsonResponse(
        {"type": "info", "message": _("You dont have permission."), "change": change}
    )
...
def task_all_bulk_delete(request):
    ...
    for task_id in ids:
        ...
        try:
            task.delete()
            messages.success(request, _("%(task)s deleted.") % {"task": task})
        except Exception as error:
            messages.error(request, error)
...
def time_sheet_delete_ajax(request, time_sheet_id):
    ...
    if time_sheet_delete_permissions(request, time_sheet_id):
        TimeSheet.objects.get(id=time_sheet_id).delete()
        return JsonResponse(
            {"type": "success", "message": _("Timesheet deleted successfully.")}
        )
    return JsonResponse(
        {
            "type": "info",
